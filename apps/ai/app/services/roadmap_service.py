...
def create_roadmap(data):
    ...
    try:
        new_roadmap = Roadmap(
            prompt_id=data.get('prompt_id')[cite: 6],
            id=data.get('id')[cite: 6],
            title=data.get('title')[cite: 6],
            description=data.get('description') [cite: 6]
        )
...
def get_roadmap_by_id(roadmap_id):
    """Lấy roadmap bằng ID (bao gồm cả các node của nó)."""
    try:
        # Dùng options để pre-load các node (tùy chọn)
        # from sqlalchemy.orm import joinedload
        # return db.session.query(Roadmap).options(joinedload(Roadmap.nodes)).get(roadmap_id)
        return db.session.get(Roadmap, roadmap_id)
    except Exception as e:
        logger.error(f"Lỗi khi lấy roadmap {roadmap_id}: {e}")
        return None

def get_roadmaps_by_user(id, page=1, per_page=20):
    ...
    try:
        ...
        logger.error(f"Lỗi khi lấy roadmaps cho user {id}: {e}")
        return None
