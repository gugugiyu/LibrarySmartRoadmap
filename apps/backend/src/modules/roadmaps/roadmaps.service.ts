...
export class RoadmapsService {
    ...
    async generate(input: GenerateRoadmapInput, userId: string) {
        ...
        // 3) với mỗi step → search + lưu recommendations + chọn default (top-1)
        for (const st of steps) {
        const query = st.keywords.join(' ')
        const hits = await this.search.hybridSearch(userId, query, { subject: input.subject }, 10)
        ...
        // 4) est timeline (đơn giản: 60 phút/step)
